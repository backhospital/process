...
label[for='data4'] { display:block; }
input { width:150px; }
textarea#data4 { width:400px; height:180px; }
...
<div id="root">
		...
		<div id="container_box">
			...
			<input type="hidden" name="idx" value="${view_approve.idx}"/>
		
			
			<div class="inputArea">
				<label for="productid">브랜드명</label>
				<span>${view_approve.productid}</span>
			</div>
			
			<div class="inputArea">
				<label for="data3">상품명</label>
				<span>${view_approve.data3}</span>
			</div>
			
			<div class="inputArea">
				<label for="data2">제안가격</label>
				<span><fmt:formatNumber value="${view_approve.data2}" pattern="###,###,###"/></span>
			</div>
					
			<div class="inputArea">
				<label for="data4">상품소개</label>				
				<div class="data4">${view_approve.data4}</div>	
			</div>
			
			<div class="inputArea">
				<label for="data1">이미지</label>
				<p>원본 이미지</p>
				<img src="${pageContext.request.contextPath }/resources/imgs/gucci_a/${view_approve.data1}" class="oriImg"/>
...
<%-- 				<img src="${goods.offerthumbimg}" class="thumbImg"/>  --%>
 			...
 			<div class="inputArea">
				<button type="button" id="display_Btn" class="btn btn-warning">병합하기</button>
<!-- 				<button type="button" id="reject_Btn" class="btn btn-danger">거부하기</button> -->
				...
				<script>
					...
					var con = confirm("정말로 display 테이블로 보내겠습니까?");
						
						if(con) {						
 							formObj.attr("action", "/admin/add_display");
							formObj.submit();
						}
					});
